...
else version (Horizon)
{
    ...
    import ys3ds.ctru._3ds.svc : ResetType;
}
else
{
...
nothrow @nogc:
    ...
    void initialize(bool manualReset, bool initialState)
    {
        version (Horizon)
        {
            ...
            LightEvent_Init(&m_event, manualReset ? ResetType.RESET_STICKY : ResetType.RESET_ONESHOT);
    ...
    */
    ...
    bool wait(Duration tmout)
    {
        ...
        {
            if (m_inited)
                LightEvent_WaitTimeout(&m_event, tmout.total!"nsecs");
        }
        else version (Windows)
        {
            if (!m_event)
